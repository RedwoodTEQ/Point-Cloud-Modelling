...
/// <summary>
/// Fork of assets/Plugins/MSAdvancedCameraController/Scripts/MSCameraController.cs
/// Asset Store page: https://assetstore.unity.com/publishers/23246
/// </summary>
    ...
    public class CameraController : MonoBehaviour
    {
        ...
        void ManageCameras()
        {
            if (changeCam)
            {
                ...
                if (cameras[index].rotationType == CameraType.TipoRotac.Stop || cameras[index].rotationType == CameraType.TipoRotac.StraightStop ||
                    cameras[index].rotationType == CameraType.TipoRotac.LookAtThePlayer || cameras[index].rotationType == CameraType.TipoRotac.FlyCamera_OnlyWindows)
                {
                    _mobileInputsIndex = 0; // 0 = all mobile inputs off
                }
                if (cameras[index].rotationType == CameraType.TipoRotac.FirstPerson || cameras[index].rotationType == CameraType.TipoRotac.ETS_StyleCamera ||
                    cameras[index].rotationType == CameraType.TipoRotac.Orbital || cameras[index].rotationType == CameraType.TipoRotac.OrbitalThatFollows)
                {
                    _mobileInputsIndex = 1; // 1 = all mobile inputs on
                }
                if (cameras[index].rotationType == CameraType.TipoRotac.FollowPlayer)
                {
            ...
            switch (cameras[index].rotationType)
            {
                ...
                case CameraType.TipoRotac.FlyCamera_OnlyWindows:
                    float xInputFly = 0.0f;
                    float yInputFly = 0.0f;

                    var setting = CameraSettings.FlyCamera_OnlyWindows;

                    bool doRotation = !setting.rotateWhenClick || Input.GetKey(setting.keyToRotate);

                    if (doRotation)
                    {
                        xInputFly = CameraSettings.FlyCamera_OnlyWindows.invertXInput ? -_horizontalInputMSACC : _horizontalInputMSACC;
                        yInputFly = CameraSettings.FlyCamera_OnlyWindows.invertYInput ? -_verticalInputMSACC : _verticalInputMSACC;
        ...
        { //use this void to change cameras using buttons
            if (Time.timeScale > 0)
            {
